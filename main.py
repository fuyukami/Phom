...
def main(page: ft.Page):
    ...
    # --- HÀM UI: TẠO NÚT 3D (STYLE TÂN CỔ ĐIỂN) ---
    def create_3d_btn(text, action, base_color="#333333", text_color="white", height=50, expand=True):
        return ft.Container(
            ...
            expand=expand,
            border_radius=8,
            # Tạo hiệu ứng 3D bằng Gradient và Shadow
            gradient=ft.LinearGradient(
                begin=ft.alignment.top_left,
                end=ft.alignment.bottom_right,
                colors=[
                    base_color,
                    ft.colors.with_opacity(0.8, base_color) # Màu nhạt hơn chút để tạo độ bóng
                ],
            ),
            border=ft.border.all(1, ft.colors.with_opacity(0.3, "white")), # Viền sáng nhẹ
            shadow=ft.BoxShadow(
                spread_radius=1,
                blur_radius=5,
    ...
    def create_player_card(p):
        ...
        # Gradient nền thẻ: Đen -> Xám than
        bg_gradient = ft.LinearGradient(
            begin=ft.alignment.top_left,
            end=ft.alignment.bottom_right,
            colors=[COLOR_CARD_BG, "#2C2C2C"]
        )
        ...
        return ft.Container(
            ...
            border=ft.border.all(1, ft.colors.with_opacity(0.2, COLOR_GOLD)), # Viền vàng mờ
            shadow=ft.BoxShadow(blur_radius=8, color=ft.colors.with_opacity(0.4, "black"), offset=ft.Offset(4,4)),
            content=ft.Row([
                # Avatar & Tên
                ft.Row([
                    ft.Icon(ft.icons.ACCOUNT_CIRCLE, size=40, color=COLOR_GOLD_DIM),
                    ft.Column([
                        ft.Text(p["name"], weight="bold", size=16, color=COLOR_GOLD),
                        ft.Container(
                            content=ft.Row([
                                ft.Icon(ft.icons.EDIT, size=12, color="grey"),
    ...
    def view_dashboard():
        ...
        # 1. Header & Thông tin chung (Khung Vàng Kim)
        header = ft.Container(
            padding=20,
        ...
        list_container = ft.Column(spacing=15, scroll=ft.ScrollMode.HIDDEN)
        for p in state["players"]:
            list_container.controls.append(create_player_card(p))
        
        # Bọc danh sách trong Container có padding
        body = ft.Container(
            content=list_container,
            padding=15,
            expand=True # Chiếm hết khoảng trống còn lại
        )
        ...
        page.add(header, body, actions)
        page.update()

    # --- CÁC MÀN HÌNH PHỤ (SỬ DỤNG GIAO DIỆN TỐI) ---
    ...
    def goto_selector(title, items, callback, multi=False):
        ...
        if not multi:
            for p in items:
                # Nút chọn người chơi dạng thẻ dài
                btn = ft.Container(
    ...
    def view_history(e):
        ...
        page.add(ft.Container(content=ft.Column(controls, scroll=ft.ScrollMode.AUTO), padding=20, expand=True))
        page.update()
